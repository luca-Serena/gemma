...
to go
  if ticks mod update_frequency = 0 [
    print-results
    set delay 0
    set deliveries 0
    ask turtles with [lora-status = "device-without-data"] [
      set lora-status "device-with-data"
      set color white
    ]
  ]
  ...
  ask turtles with [lora-status = "device-with-data"] [
    if communicate = True [
      ;show "communication successful"
      set lora-status "device-without-data"
      set color grey
      set deliveries deliveries + 1
      set delay delay + ticks mod update_frequency
      show (word "success at " ticks)
    ]
  ]
...
to update-vehicles [lora-vehicles]
  let diff-lora    lora-vehicles  -  lora-total

  if diff-lora > 0 [
    ask n-of diff-lora turtles with [lora-status = "NO"][set lora-status  "YES"]
  ]

  ask turtles with [lora-status = "YES"] [set color green]
  set lora-total lora-vehicles
end


to-report communicate
  let agents-in-range other turtles with [lora-status = "YES"] in-radius 5
  report any? agents-in-range
end
...
to print-results
  show (word "delay: " delay)
  show (word "deliveries: " deliveries)
end
